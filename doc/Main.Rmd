...
We performed a stochastic gradien descent to do matrix factorization. The algorithm should consider case that there are new users and movies adding to the dataset we used to train. In other words, the dimension the matrix R, q, p is dynamic.
```{r}
U <- length(unique(data$userId))
I <- length(unique(data$movieId))
source("../lib/ALS.R")
...
After matrix factorization, post-processing will be performed to improve accuracy.
```{r, eval= FALSE}
...
- As we can observe from the table above, Model 2 has a lower RMSE value for train dataset (0.591) compared to Model 1 (0.793).
- However, Model 1 performs better for test dataset based on the test RMSE value of 0.873 vs. model 2 of 1.344. Therefore, we can conclude that **Model 1 (with regularization) has a better fit for test data compared to model 2 (withour regularization)**.
